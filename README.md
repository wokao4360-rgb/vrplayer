...
- [2026-03-11 17:00:17] GitHub Pages 项目子路径（如 `/vrplayer/`）下，不能再假设站点根路径就是应用根路径：`public/config.json` 里的 `/assets/...` 运行时必须先按当前应用基路径重写成 `/vrplayer/assets/...`，`sw.js` 也必须按当前应用基路径注册，不能写死 `/sw.js`；否则首页封面、场景缩略图、全景资源或 SW 注册都会在线上命中站点根目录 404。
