...
- [2026-03-11 17:00:17] GitHub Pages 项目子路径（如 `/vrplayer/`）下，`public/config.json` 里的 `/assets/...` 不能直接当站点根路径使用；运行时必须先按当前应用基路径重写成 `/vrplayer/assets/...`，再交给 CDN/图片代理链路，否则首页封面、场景缩略图和全景资源会在线上命中站点根目录 404。
