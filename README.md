...
- [2026-03-15 20:40:00] `linzexu/south_gate` ?? `equirect z0-z3` AVIF/KTX2 ???? `cubemap low/high` ????????????????????????????/??????????????? `public/assets/panos/tiles/linzexu/south_gate/manifest.json` ?? `equirect-tiles`?????? cubemap ?????????
